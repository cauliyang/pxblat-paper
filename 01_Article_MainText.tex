...
This study details the design, implementation, and evaluation of PxBLAT.
...
% Our exploration begins with the study of the BLAST-Like Alignment Tool (BLAT), a standout in the bioinformatics field for its capability to perform rapid genome sequence alignments.
% Kent first introduced BLAT in 2002, offering a faster alternative to Basic Local Alignment Search Tool (BLAST) for aligning DNA sequences to the human genome [Ref].
% Over the years, several studies have reinforced the efficacy of BLAT in various applications, ranging from gene mapping to whole-genome alignments [Ref1, Ref2, Ref3].


% However, despite BLAT's performance and wide utilization, limitations exist in its interface and integration with other bioinformatics tools.
% These issues have been sporadically addressed in literature, mainly through the development of wrappers or libraries attempting to augment BLAT's functionality [Ref4, Ref5].
% Yet, these solutions often provide only partial or inefficient integration, lacking a holistic approach to enhance both the performance and the usability of BLAT.
%

% On the other hand, Python's rise as a favored programming language in bioinformatics is well-documented, due to its ease of use, extensive libraries, and versatility [Ref6].
% Various binding libraries have been developed to extend Python's reach into other computing languages, improving the flexibility and interoperability of bioinformatics tools [Ref7, Ref8].
% For instance, Biopython, one of the most prominent bioinformatics libraries, provides interfaces to tools like BLAST, Clustal, and others [Ref9].
% However, to date, no comprehensive Python binding library for BLAT has been developed.
%

% It is against this backdrop that our research into PxBLAT takes place.
% The intent is to fill the observed gap by providing a Python binding library specifically tailored for BLAT, addressing both its efficiency and ergonomic concerns.
% This study is one of the first to take a holistic approach, potentially contributing to more streamlined, efficient genomic analysis workflows.

% Please note that the "[Ref]" markers are placeholders for actual references.
% Be sure to replace them with appropriate citations corresponding to the points discussed.



% What are the differences between BLAT ~\citep{kent2002blat} and BLAST ~\citep{altschul1990basic}?
...
% By default, the index consists of all non-overlapping 11-mers except for those heavily involved in repeats, and it uses less than a gigabyte of RAM.
...
% (The default settings and expected behavior of standalone Blat are slightly different from those on the graphical version of BLAT.
% )

% On proteins, BLAT uses 4-mers rather than 11-mers, finding protein sequences of \SI{80}{\percent}  and greater similarity to the query of length 20+ amino acids.
% The protein index requires slightly more than 2 gigabytes of RAM.
% In practice -- due to sequence divergence rates over evolutionary time -- DNA BLAT works well within humans and primates, while protein Blat continues to find good matches within terrestrial vertebrates and even earlier organisms for conserved proteins.
% Within humans, protein Blat gives a much better picture of gene families (paralogs) than DNA Blat.
...
% The overarching goal in the development of PxBLAT was to create an efficient, ergonomic Python binding library that enhances the interface and usage of the BLAT tool.
